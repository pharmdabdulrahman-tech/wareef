...
export function Refined() {
  ...
  return (
    <div className="min-h-screen customer-app ghars wareef-customer" dir={english ? "ltr" : "rtl"} data-preview-view={view}>
      <div className="shell">
        <main className="screen needs-hub hub-refined">
          ...
          <div className="needs-grid">
            <button className="need-card services" onClick={() => setView("services")}>
              ...
              <span className="need-copy"><b>{text("الخدمات", "Services")}</b><small>{text("احجز عناية لمساحتك", "Book care for your space")}</small></span>
              <i aria-hidden="true"><DirectionIcon /></i>
            </button>
            <button className="need-card products" onClick={() => setView("products")}>
              <span className="need-icon"><Leaf /></span>
              <span className="need-copy"><b>{text("المنتجات", "Products")}</b><small>{text("نباتات وأصص ومستلزمات", "Plants, pots & essentials")}</small></span>
              <i aria-hidden="true"><DirectionIcon /></i>
            </button>
            <button className="need-card subscriptions" disabled>
              <span className="need-icon"><Heart /></span>
              <span className="need-copy"><b>{text("الاشتراكات", "Subscriptions")}</b><small>{text("غير متاحة حتى تكتمل تفاصيل التشغيل", "Unavailable until operating details are finalized")}</small></span>
